...
class User(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.pushButton_4.setGeometry(QtCore.QRect(100, 130, 161, 22))
    ...
    def change(self):
        ...
        try:
            ...
            cur = db.cursor()
            ch1 = """UPDATE user SET login = %s, password = %s WHERE ID_User = %s"""
            ch2 = (login, password, ID)
            cur.execute(ch1, ch2)
            db.commit()
            cur.close()
        finally:
    ...
    def dels(self):
        ...
        try:
            ...
            return
        cur.execute("DELETE FROM user WHERE ID_User = %s",(int(ID),))
        db.commit()
        cur.close()
        db.close()
